...
- [ ] T001 Create top-level directory layout per plan.md: `services/`, `packages/`, `tests/mocks/`, `infra/` at repo root (no top-level `mock-services/` — mocks live under `tests/`)
- [ ] T002 Create `docker-compose.yml` with all 12 services (qdrant, postgres, redis, temporal, minio, prometheus, ollama, agent-workers, context-hydrator, knowledge-ingestion, connector-registry, and two test-only mocks added only in `docker-compose.test.yml`); pin all image tags; add named volumes for qdrant, postgres, redis, minio; add `ENVIRONMENT` and `OLLAMA_HOST` env vars on every application service
- [ ] T003 [P] Add GPU reservation block (`deploy.resources.reservations.devices`) and health check to `ollama` service in `docker-compose.yml`
- [ ] T004 [P] Add health checks and `depends_on: condition: service_healthy` for all application services on their infra dependencies in `docker-compose.yml`
- [ ] T005 [P] Create `docker-compose.test.yml` skeleton: inherits base Compose, overrides `ENVIRONMENT=test`, replaces GitHub/Confluence/Slack with `github-api-mock` (port 8085) and `confluence-api-mock` (port 8084)
- [ ] T006 [P] Create `Dockerfile` for `services/agent-workers/` using `python:3.11-slim`; `uv sync --frozen`; CMD runs Temporal worker
- [ ] T007 [P] Create `Dockerfile` for `services/context-hydrator/` using `python:3.11-slim`; `uv sync --frozen`; expose port 8081
- [ ] T008 [P] Create `Dockerfile` for `services/knowledge-ingestion/` using `python:3.11-slim`; `uv sync --frozen`; CMD runs Temporal worker
- [ ] T009 [P] Create `Dockerfile` for `services/connector-registry/` using `golang:1.22-alpine` multi-stage build; final stage `gcr.io/distroless/static`; expose port 8082
- [ ] T010 [P] Create `pyproject.toml` + `uv.lock` stub for `packages/memrag-shared/` (no external deps initially); create `packages/memrag-shared/src/memrag_shared/__init__.py`
- [ ] T011 [P] Create `.env.example` with all required variables: `ENVIRONMENT`, `WORKSPACE_ID`, `POSTGRES_PASSWORD`, `MINIO_ROOT_PASSWORD`, `GITHUB_TOKEN`, `CONFLUENCE_BASE_URL`, `TEMPORAL_HOST`, `QDRANT_HOST`, `REDIS_URL`, `OLLAMA_HOST`, `OLLAMA_DEVICE`
...
- [ ] T012 Create `packages/memrag-shared/src/memrag_shared/manifest.py`: `AgentManifest` dataclass with fields `agent_id`, `workspace_id`, `domain` (`code|ops|policy|data|None`), `knowledge_top_k` (default 8), `context_token_budget`, `promote_to_shared` (bool), `knowledge_source_filter`, `agent_tags`
- [ ] T013 [P] Create `packages/memrag-shared/src/memrag_shared/weights.py`: reads 6×4 domain weight matrix from `WEIGHTS_CONFIG_SOURCE` env var (supports `aws-appconfig` or `env-file`); if `aws-appconfig`: fetches from AWS AppConfig application/profile; if `env-file`: reads from `.weights.json` or `$WEIGHTS_FILE` path; falls back to defaults from FR-026; `get_weight(source_type, domain) -> float` helper that queries the loaded config (returns 1.0 when domain is None); includes reload mechanism for AppConfig (watch for config version changes via polling or event-based)
- [ ] T014 [P] Create `packages/memrag-shared/src/memrag_shared/layers.py`: constants `LAYER_SESSION=1`, `LAYER_AGENT=2`, `LAYER_SHARED=3`, `LAYER_ORG=4`; `MemoryChunk` and `KnowledgeChunk` dataclasses matching `contracts/context_hydration.md`
- [ ] T015 Create `services/connector-registry/migrations/` with Goose SQL files for all 5 PostgreSQL tables: `001_knowledge_connectors.sql`, `002_knowledge_sync_state.sql`, `003_knowledge_sharing_grants.sql`, `004_pii_audit_log.sql`, `005_workflow_executions.sql` — exact DDL from `data-model.md`
- [ ] T016 Create `services/connector-registry/cmd/migrate/main.go`: runs Goose migrations on startup via `connector-registry` Compose service `command`; reads `DATABASE_URL` from env
- [ ] T017 [P] Create `services/agent-workers/src/infra/qdrant_client.py`: thin wrapper around `qdrant-client`; reads `QDRANT_HOST` from env; exposes `get_client() -> QdrantClient`
...
- [ ] T019 [P] Create `services/agent-workers/src/infra/redis_client.py`: Redis connection factory reading `REDIS_URL`; `session_key(workspace_id, session_id, field) -> str` helper returning correct key pattern from `data-model.md`; `grants_key(workspace_id) -> str`
- [ ] T020 [P] Create `services/agent-workers/src/infra/ollama_client.py`: thin async HTTP client reading `OLLAMA_HOST`; `embed(texts: list[str]) -> list[list[float]]` (calls `POST /api/embeddings`, model `qwen3-embedding:4b`); `complete(messages: list[dict]) -> str` (calls `POST /api/chat`, model `llama3.1:8b`); validates `OLLAMA_HOST` is reachable on service startup with a `GET /api/version` health probe
- [ ] T021 [P] Create `services/agent-workers/src/infra/temporal_client.py`: Temporal client factory reading `TEMPORAL_HOST`; `get_client() -> Client`; `get_worker(task_queue="agent-workers") -> Worker`
- [ ] T022 [P] Create `services/knowledge-ingestion/src/infra/temporal_client.py`: same pattern as T021 but task queue `"ingestion-workers"`; also creates `services/knowledge-ingestion/src/infra/ollama_client.py` (same thin Ollama client as T020)
- [ ] T023 Create `services/connector-registry/internal/db/`: `go.mod` (module `memrag/connector-registry`); `sqlc.yaml` config; hand-write or sqlc-generate typed queries for `knowledge_connectors` CRUD and `knowledge_sharing_grants` CRUD matching `contracts/connector_management_api.md`
- [ ] T024 [P] Create `services/knowledge-ingestion/src/infra/iceberg_client.py`: PyIceberg `load_catalog()` pointing at MinIO (`s3://memrag-archive/`); `get_tombstone_table() -> Table`; creates table if absent using `data-model.md` schema (partitioned by `workspace_id, days(tombstoned_at)`)
...
- [ ] T025 [US1] Create `services/agent-workers/src/activities/session.py`: `fetch_recent_session(workspace_id, session_id) -> list[Turn]` activity — reads `{workspace_id}:session:{session_id}:messages` from Redis; returns empty list on miss (no error); sets 24h TTL on key read
- [ ] T026 [US1] Create `services/agent-workers/src/activities/session.py` (append): `checkpoint_session(workspace_id, session_id, turns: list[Turn])` activity — serialises turns to JSON, writes to Redis key, refreshes 24h TTL; if payload > 256KB, stores via Redis reference pointer (stores bytes at `{workspace_id}:session:{session_id}:payload:{idx}`) and records pointer list in the messages key
- [ ] T027 [US1] Create `services/agent-workers/src/workflows/agent_workflow.py`: `AgentWorkflow` Temporal workflow class; `run(params: WorkflowParams)` method with activity sequence: `fetch_recent_session` → (placeholder for recall fan-out) → (placeholder for LLM call) → `checkpoint_session`; workflow signals `hitl_response`
- [ ] T028 [US1] Add `services/agent-workers/src/worker.py`: registers `AgentWorkflow` and all activities; starts Temporal worker on `TEMPORAL_HOST` with task queue `"agent-workers"`; starts as Compose service CMD
- [ ] T029 [US1] Create `services/agent-workers/tests/integration/test_session_buffer.py`: starts a sandboxed Temporal test env; runs `AgentWorkflow` with 12 turns; sends `SIGKILL` to the activity mid-execution via sandbox; resumes workflow; asserts reconstructed turns equal original 12 turns and session key TTL is ≥ 23h
- [ ] T030 [US1] Create `services/agent-workers/tests/unit/test_session_keys.py`: asserts `session_key()` and `grants_key()` return exact key patterns from `data-model.md` Redis Key Schema section
...
- [ ] T031 [US2] Create `services/agent-workers/src/memory/mem0_client.py`: wraps `mem0ai.Memory` SDK; `extract_and_store(agent_id, workspace_id, text) -> list[str]` (fact IDs); unit-test cases disables LLM extraction and stores the raw text as-is for deterministic tests; reads Qdrant host from env; uses Mem0 for extract + store only. `ENVIRONMENT=test` != unit-test cases.qwen3-embedding:8b
- [ ] T033 [US2] Create `services/agent-workers/src/recall/layer2.py`: `recall_agent_memory(workspace_id, agent_id, query_text, top_k=8) -> list[MemoryChunk]`; embeds query via `ollama_client.embed()` (calls Ollama `qwen3-embedding:4b` directly); queries Qdrant `agent_memories` with named-vector hybrid search (`dense` + `sparse` BM25, fused via Qdrant `prefetch` + `query` RRF); filters on `workspace_id`, `agent_id`, `tombstoned=false`
- [ ] T034 [US2] Create `services/agent-workers/src/memory/dedup.py`: `is_near_duplicate(new_embedding, workspace_id, agent_id, threshold=0.95) -> bool`; runs a nearest-neighbour query against `agent_memories`; returns True if any result has cosine similarity ≥ 0.95; called by `extract_and_store` before Qdrant upsert
- [ ] T035 [US2] Create `services/agent-workers/src/activities/memory.py`: `store_agent_memory(workspace_id, agent_id, workflow_output)` activity — calls `extract_and_store`; fire-and-forget from `AgentWorkflow` (scheduled async, does not block response path); adds `last_accessed_at` update on every successful recall hit (upsert payload field)
- [ ] T036 [US2] Update `AgentWorkflow` in `services/agent-workers/src/workflows/agent_workflow.py` to include `recall_agent_memory` in the recall slot (replaces placeholder from T027); schedule `store_agent_memory` as detached/fire-and-forget after Ollama completion call
- [ ] T037 [US2] Create `services/knowledge-ingestion/src/workflows/decay_memories.py`: `DecayMemoriesWorkflow` Temporal cron workflow (schedule `"0 2 * * *"`); activity `decay_and_archive(workspace_id)` — scroll `agent_memories` by batch; recompute `decay_score` (linear: `score * exp(-days_inactive / half_life)`, half_life=90 for episodic, 365 for semantic); bulk-update payload; for `decay_score < 0.1`: write row to S3 Iceberg tombstone table, delete point from Qdrant
- [ ] T038 [US2] Register `DecayMemoriesWorkflow` in `services/knowledge-ingestion/src/worker.py`; add cron schedule on worker startup
- [ ] T039 [US2] Create `services/agent-workers/tests/integration/test_long_term_memory.py`: runs workflow with output text; runs second workflow with semantically similar query; asserts second workflow context includes fact from first; runs second workflow again with identical output; asserts Qdrant point count unchanged (dedup); asserts decay workflow sets score < 0.1 on artificially aged entries
...
- [ ] T040 [US3] Create `services/agent-workers/src/recall/layer3.py`: `recall_shared_memory(workspace_id, query_text, top_k=8) -> list[MemoryChunk]`; hybrid Qdrant search against `shared_memories`; filters strictly on `workspace_id`; uses same dense+sparse RRF pattern as Layer 2
- [ ] T041 [US3] Create `services/agent-workers/src/memory/shared.py`: `promote_to_shared(workspace_id, source_agent_id, text, embedding)` — upserts point to `shared_memories` with `workspace_id`, `source_agent_id`, `promoted_at`, `content_hash`; checks dedup (0.95 threshold) against existing `shared_memories` before upsert
- [ ] T042 [US3] Create `services/agent-workers/src/tools/promote_finding.py`: LLM-callable tool `promote_finding_to_shared_knowledge(text: str)`; wraps `promote_to_shared`; registered as Temporal activity callable from workflow tool-use loop; returns `{"status": "stored" | "duplicate"}`
- [ ] T043 [US3] Update `AgentWorkflow` to: (a) if `manifest.promote_to_shared=True`, call `promote_to_shared` on workflow completion (auto-promotion path); (b) support `promote_finding_to_shared_knowledge` tool in the reasoning loop (explicit path); add `recall_shared_memory` to the parallel fan-out started in T035
...
- [ ] T045 [US4] Create `services/connector-registry/internal/api/server.go`: chi router; register handlers for `POST /v1/connectors`, `GET /v1/connectors`, `GET /v1/connectors/{id}`, `PATCH /v1/connectors/{id}`, `DELETE /v1/connectors/{id}`, `GET /v1/connectors/{id}/status`; reads `X-Workspace-ID` header for all requests
- [ ] T046 [P] [US4] Create `services/connector-registry/internal/api/handlers_connector.go`: implement all five connector CRUD handlers using sqlc-generated DB queries from T023; `POST` sets `sync_status="pending"` and enqueues `IngestionWorkflow` via Temporal client; `DELETE` enqueues Qdrant background cleanup task
- [ ] T047 [P] [US4] Create `services/connector-registry/internal/temporal/client.go`: Temporal Go client factory; `SignalWorkflow(runID, signalName, payload)` helper for HITL endpoint (T064); `StartIngestionWorkflow(connectorID, workspaceID)` helper
- [ ] T048 [US4] Create `services/knowledge-ingestion/src/connectors/base.py`: `BaseConnector` abstract class with `authenticate()`, `list_resources() -> list[Resource]`, `fetch_resource(resource_id) -> bytes` abstract methods; `Resource` dataclass with `id`, `url`, `title`, `last_modified`
- [ ] T049 [P] [US4] Create `services/knowledge-ingestion/src/connectors/github.py`: `GitHubConnector(BaseConnector)` — reads `GITHUB_TOKEN` (or `GITHUB_API_BASE_URL` for mock); `list_resources`: GitHub Trees API for configured repo+branch+extensions; `fetch_resource`: Contents API; respects `ENVIRONMENT=test` by pointing at `github-api-mock:8085`
- [ ] T050 [P] [US4] Create `services/knowledge-ingestion/src/connectors/confluence.py`: `ConfluenceConnector(BaseConnector)` — OAuth 2.0 3-LO token exchange via `CONFLUENCE_BASE_URL`; `list_resources`: CQL search `space IN (...)` with `lastModified > {last_sync}`; `fetch_resource`: page content endpoint; respects `ENVIRONMENT=test` by pointing at `confluence-api-mock:8084`
- [ ] T051 [P] [US4] Create `services/knowledge-ingestion/src/connectors/slack.py`: `SlackConnector(BaseConnector)` — `list_resources`: conversations.history for configured channels; hard-filter: `message.ts < (now - 7 days)`; never fetches messages < 7 days old (FR-015, A-009)
- [ ] T052 [P] [US4] Create `services/knowledge-ingestion/src/connectors/rds_schema.py`: `RDSSchemaConnector(BaseConnector)` — connects via `psycopg2`; `list_resources`: queries `information_schema.tables`; `fetch_resource`: fetches columns, data types, column comments, foreign keys for one table; NEVER queries row data
- [ ] T053 [US4] Create `services/knowledge-ingestion/src/chunker.py`: `chunk(text, source_type, content_type) -> list[str]`; code files → tree-sitter AST chunking at function/class boundaries; prose → chonkie semantic chunking with overlap; RDS schema → one chunk per table template (table name + column defs + FK summary)
- [ ] T054 [US4] Create `services/knowledge-ingestion/src/embedder.py`: `embed_batch(texts: list[str]) -> list[list[float]]`; calls Ollama `qwen3-embedding:4b` directly via `ollama_client.embed()` (no intermediate gateway); returns list of 768-dim float32 vectors; also `embed_sparse(texts) -> list[dict]` using BM25 tokenisation for sparse vector weights
- [ ] T055 [US4] Create `services/knowledge-ingestion/src/workflows/ingestion.py`: `IngestionWorkflow` Temporal workflow; activities: `fetch_resources(connector_id)` → `diff_resources(connector_id, resources)` (content-hash comparison vs `knowledge_sync_state`) → for each changed: `chunk_and_embed(resource)` → `pii_screen(chunks)` → `upsert_org_knowledge(connector_id, chunks)` → `update_sync_state(connector_id, resource_id, content_hash)`; supports `full_sync` and `delta_sync` modes
- [ ] T056 [US4] Create `services/knowledge-ingestion/src/activities/sync_state.py`: `diff_resources` — queries `knowledge_sync_state` table via PostgreSQL; returns only resources where `content_hash != stored_hash` or resource is new; `update_sync_state` — upserts `(connector_id, resource_id, content_hash)` after successful ingestion
- [ ] T057 [US4] Create `services/knowledge-ingestion/src/activities/upsert.py`: `upsert_org_knowledge(connector_id, chunks_with_embeddings, connector_config)` — upserts Qdrant `org_knowledge` points with full payload from `data-model.md` (workspace_id, source_type, sharing_scope, agent_scope, etc.); uses `content_hash` as deterministic point ID for idempotency
- [ ] T058 [US4] Create `tests/mocks/github-api-mock/`: FastAPI app implementing GitHub REST Trees API (`GET /repos/{owner}/{repo}/git/trees/{sha}?recursive=1`) and Contents API (`GET /repos/{owner}/{repo}/contents/{path}`); returns synthetic repo fixture with 10 Python files; push webhook simulator endpoint; Dockerfile in same directory
- [ ] T059 [US4] Create `tests/mocks/confluence-api-mock/`: FastAPI app implementing: OAuth 2.0 authorization endpoint (`GET /oauth/authorize`), token exchange (`POST /oauth/token`), token refresh (`POST /oauth/token` with `grant_type=refresh_token`), CQL search (`GET /rest/api/content/search?cql=...`), page content (`GET /rest/api/content/{id}?expand=body.storage`); returns 5 synthetic pages; implements full 3-LO flow per A-018 and FR-031; Dockerfile in same directory
- [ ] T060 [US4] Create `services/knowledge-ingestion/tests/integration/test_byod_pipeline.py`: creates GitHub connector in ENVIRONMENT=test pointing at github-api-mock; triggers IngestionWorkflow; polls until `sync_status=ok`; asserts ≥10 Qdrant points in `org_knowledge` with correct `workspace_id` and `connector_id`; triggers delta sync with no changes; asserts no new points added (idempotency via content-hash)

**Checkpoint**: `docker compose exec knowledge-ingestion pytest tests/integration/test_byod_pipeline.py` passes. Full BYOD pipeline: connector → ingest → chunk → embed → upsert → verify.
...
- [ ] T061 [US5] Create `services/knowledge-ingestion/src/pii.py`: `PIIScanner` class; initialises `presidio_analyzer.AnalyzerEngine` with all 12 entity recognisers (EMAIL, PHONE, PERSON_NAME, PAN, UPI_ID, DEMAT_ID, CREDIT_CARD, BANK_ACCOUNT, IP_ADDRESS, HEALTH_INFO, PASSWORD, SECRET); `scan(chunk_text, pii_config) -> PIIResult`; apply hard rules first (CREDIT_CARD/BANK_ACCOUNT → redact; PASSWORD/SECRET → drop); apply configurable actions for remaining entities; return sanitised text or DROP sentinel; never log raw detected values
...
- [ ] T077 Complete `docker-compose.test.yml`: add `github-api-mock` (from `tests/mocks/github-api-mock/`) and `confluence-api-mock` (from `tests/mocks/confluence-api-mock/`) services; override `GITHUB_API_BASE_URL`, `CONFLUENCE_BASE_URL`, `SLACK_API_BASE_URL` env vars on `knowledge-ingestion` to point at mocks; set `ENVIRONMENT=test` on all application services; add `app` service that runs `pytest tests/` and exits
- [ ] T078 Create end-to-end integration test `tests/e2e/test_full_stack.py` that validates: (a) connector create → ingest → recall chain; (b) PII detection halt + HITL approve; (c) scope change propagates within 60s; (d) full AgentWorkflow with all four layers; run via `docker compose -f docker-compose.test.yml up --exit-code-from app`
- [ ] T079 [P] Verify `.env.example` has entries for every env var referenced across all service code; fill in any gaps discovered during T076–T078
- [ ] T080 [P] Run through `quickstart.md` steps in a clean environment; update any command that fails or has changed since plan; confirm `docker compose ps --format "table {{.Name}}\t{{.Status}}"` shows all healthy
...
**Phase 6 (US4)**: T045 first; T049/T050/T051/T052 in parallel (four connectors); T053 and T054 after T048; T058/T059 in parallel with connector implementations  
**Phase 7 (US5)**: T060 → T061 → T062; T063 can run in parallel with T060–T062  
**Phase 10**: T076/T079/T080 in parallel after T077/T078
