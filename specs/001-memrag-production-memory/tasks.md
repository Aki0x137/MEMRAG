...
- [ ] T016 Create `services/connector-registry/cmd/migrate/main.go`: runs Goose migrations on startup via `connector-registry` Compose service `command`; reads `DATABASE_URL` from env; initialises shared AWS SDK config (`aws-sdk-go-v2/config`) so later subcommands can reuse region/credential resolution consistently
...
- [ ] T052 [P] [US4] Create `services/knowledge-ingestion/src/connectors/rds_schema.py`: `RDSSchemaConnector(BaseConnector)` — resolves connection metadata from connector config and AWS Secrets Manager reference; supports standard PostgreSQL credentials and optional AWS RDS IAM auth token generation via `boto3`; connects via `psycopg2`; `list_resources`: queries `information_schema.tables`; `fetch_resource`: fetches columns, data types, column comments, foreign keys for one table; NEVER queries row data
