...
- [x] T002 Create `docker-compose.yml` with all 12 services (qdrant, postgres, redis, temporal, minio, prometheus, ollama, agent-workers, context-hydrator, knowledge-ingestion, connector-registry, and two test-only mocks added only in `docker-compose.test.yml`); pin all image tags; add named volumes for qdrant, postgres, redis, minio; add `ENVIRONMENT` and `OLLAMA_HOST` env vars on every application service
...
- [x] T010 [P] Create `pyproject.toml` + `uv.lock` stub for `packages/memrag-shared/` (no external deps initially); create `packages/memrag-shared/src/memrag_shared/__init__.py`
- [x] T011 [P] Create `.env.example` with all required variables: `ENVIRONMENT`, `WORKSPACE_ID`, `POSTGRES_PASSWORD`, `MINIO_ROOT_PASSWORD`, `GITHUB_TOKEN`, `CONFLUENCE_BASE_URL`, `TEMPORAL_HOST`, `QDRANT_HOST`, `REDIS_URL`, `OLLAMA_HOST`, `OLLAMA_DEVICE`
...
- [ ] T029 [US1] Create `services/agent-workers/tests/integration/test_session_buffer.py`: starts a sandboxed Temporal test env; runs `AgentWorkflow` with 12 turns and 500KB large-payload workflow input; sends `SIGKILL` to the activity mid-execution via sandbox; resumes workflow; asserts reconstructed turns equal original 12 turns, external payloads are correctly fetched by pointer, and session key TTL is ≥ 23h
...
- [ ] T081 [P] Verify `.env.example` has entries for every env var referenced across all service code; fill in any gaps discovered during T078–T080
