"""Shared workspace memory — promote findings to shared_memories Qdrant collection."""
...
def _shared_point_id(workspace_id: str, source_agent_id: str, text: str) -> str:
    digest = hashlib.sha256(f"{workspace_id}:{source_agent_id}:{text}".encode("utf-8")).hexdigest()
    return digest
...
async def promote_to_shared(
    ...
    """Upsert a finding into shared_memories. Returns 'stored' or 'duplicate'."""
